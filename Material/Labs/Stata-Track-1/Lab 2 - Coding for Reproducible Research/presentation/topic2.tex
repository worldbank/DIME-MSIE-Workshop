...
\begin{frame}{Lab Task 3}
	...
	\begin{itemize}
		\item Use the same code and create the variable \texttt{inc\_01\_EUR0}. (1 RWF= .0010 Euro.) Remember to also label it!
		\item Create a dummy variable for high on-farm enterprise income in the same way \texttt{high\_income} was created. Use the cut of 100  Euros. Remember to label it!
	\end{itemize}
\end{frame}
...
\begin{frame}{Lab Task 6}
	\begin{itemize}
		\item Make sure that you have the \texttt{endline\_data\_raw.dta} open.
		\begin{itemize}
		...
		\item Another way to investigate the missing observations is to tabulate variable \texttt{education\_1}.
		\begin{itemize}
			\item Test first: \texttt{tabulate inc\_zero}
			\item Then test: \texttt{tabulate inc\_zero, missing}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Lab Task 8 - MAKE SMALLER}
	\begin{itemize}
		\item Use the drop down menus to import the \texttt{village\_codes.xls} data to Stata. After you select Excel Spreadsheet you get a new window with all import excel options. For now, only use browse to select the file and then click OK.
		\begin{itemize}
			\item Use \texttt{describe}, \texttt{tabulate}, \texttt{browse} or any other tools you have learned to explore the data. What are your key observations?
		\end{itemize}
		...
		\end{itemize}
		\item In the result window or in the review window, see that the two lines of code generated are similar, but have the difference that the second time the word \texttt{firstrow} is used at the end.
