...
\begin{frame}{Task 2 - Update the topic 2 do-file}
	\begin{itemize}
		\item Move the data set \texttt{endline\_data\_raw.dta} to the \texttt{ST1/DataSets/Deidentified} folder.
		\item In your do-file for topic 2, where you have \newline \texttt{use "\$\string{folder\_Lab1\string}/Data/endline\_data\_raw.dta", clear} replace the global to use the \texttt{\$\string{ST1\_dtDeID\string}} global created in your master do-file, and remove the \texttt{Data/} part of the path, i.e. \newline  \texttt{use "\$\string{ST1\_dtDeID\string}/endline\_data\_raw.dta", clear}
		\item Similarly, replace the global in \newline \texttt{save "\$\string{folder\_Lab1\string}/Data/endline\_data\_post\_lab2.dta", replace} to the \texttt{\$\string{ST1\_dtInt\string}} global, i.e. \newline  \texttt{save "\$\string{ST1\_dtInt\string}/endline\_data\_post\_lab2.dta", replace}
	\end{itemize}
...
\begin{frame}{Task 3 - Test of unit of observation}
	\begin{itemize}
		...
		\end{itemize}
		\item Create a new do-file for this session. Save it in the same location as the do-file from topic 2, and then make sure that the new do-file also runs from the master do-file. 
		\item Then in that do-file, open the data using \texttt{use "\$\string{ST1\_dtInt\string}/endline\_data\_post\_lab2.dta", clear}
...
\begin{frame}{Task 7}
	...
	\codeexample{save-topic3.do}{code/save-topic3.do}
\end{frame}
