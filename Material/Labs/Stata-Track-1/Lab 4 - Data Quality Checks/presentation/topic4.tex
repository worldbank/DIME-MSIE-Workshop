...
Alternative installation fir WB computers
...
\begin{frame}{IPA's HFC: Folder set-up}
	...
	Then set up a folder for this exercise and create a global for that folder. And after that run the set up command \texttt{ipacheck} to that folder. You can use nay name in the option \texttt{surveys()}
...
\begin{frame}{IPA's HFC: Add more tests}
	...
	\begin{itemize}
		\item In \texttt{6. logic}, add \texttt{unit\_harvest\_1\_1\_1 amount\_harvest\_1\_1\_1} in the variable column, \texttt{unit\_harvest\_1\_1\_1 == "N/A"} in the assert column, and \texttt{amount\_harvest\_1\_1\_1 == 0} in the if-statement column.
		...
		\item In \texttt{8. constraints}, add all \texttt{pl\_aga\_01},\texttt{pl\_aga\_02},...,\texttt{pl\_aga\_15}, 01-15, and 0 as \texttt{hard\_min} and \texttt{soft\_min}, 90 as \texttt{soft\_max} and 150 as \texttt{hard\_max},
		\item In \texttt{11. outliers}, add all the \texttt{inc\_01},\texttt{inc\_02},...,\texttt{inc\_17} variables. See that both variable and multiplier is green so both are required. Set all multipliers to 2.
	\end{itemize}
\end{frame}

\begin{frame}{IPA's HFC: Add more tests}	
	\begin{itemize}
		\item Then go to the \texttt{6. logic} sheet and select all tests we are now using (see image below)
