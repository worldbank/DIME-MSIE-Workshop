...
\begin{frame}{Why do we have duplicates?}
	\begin{itemize}
		...
		\begin{itemize}
			\item Often we want to track these visits. So we start the interview on the tablet, but then have a question where the enumerator can indicate that the interview was not started, after which that data is submitted.
		\end{itemize}	
	\end{itemize}
\end{frame}
...
\begin{frame}{IPA's HFC: Set-up hfc\_inputs.xlsm}
	...
	\begin{columns}[c]
		...
		\column{.22\linewidth}
		\small Next, open up the excel file \texttt{hfc\_inputs.xlsm} in \texttt{04\_checks/01\_inputs}. 
